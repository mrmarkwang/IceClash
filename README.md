...
WASD and the fixed, always-visible lower-left joystick never aim passes or shots. During human possession, the touch actions are PASS, DEKE, and SHOOT. When the puck becomes loose, control moves once to the closest Blue skater and only SWITCH remains visible; a manual loose-puck switch persists while the puck continues moving. When a Red skater establishes possession, the actions become exactly SWITCH and CHECK. The joystick remains unchanged. SWITCH uses the same useful-player selection as Q/gamepad switching. CHECK chooses a close body check or a longer forward pull check from the controlled Blue skater. A successful check dislodges the puck into normal free physics and never grants possession directly.
