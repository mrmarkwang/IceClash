...
Deliver a fully playable local PvE match in Unity for mobile landscape play. The human team and AI team each field five skaters and one AI goalie. The user controls one human-team skater at a time while AI controls every other player. A complete match must support skating, physics-based possession, assisted passing, charged shooting, useful player switching, believable imperfect team AI, goalie saves, scoring, faceoff resets, a timer, match results, a hockey camera, and uncluttered touch controls.
...
- [x] `PrototypeArena` launches a local match containing five Blue skaters plus one Blue goalie, five Red skaters plus one Red goalie, one independent Rigidbody puck, two goals, and a clear small marked rink built from placeholder geometry.
- [x] Exactly one Blue skater is human-controlled at a time; the other Blue skaters and all Red skaters are AI-controlled, and roster construction remains count-driven so future roster-size changes do not require rewriting gameplay systems.
...
- [x] The left joystick and WASD affect player movement only and never supply pass, shot, or target-selection direction.
- [x] While the human-controlled player possesses the puck, the game continuously recommends one teammate using facing, distance, openness, lane obstruction, interception risk, and offensive progress without reading movement input as an aiming signal.
- [x] The recommended pass displays a subtle dotted projected path in the rink from the puck toward the recommendation; the path updates as play changes and disappears immediately when human possession ends or no target is available.
- [x] Tapping PASS immediately releases an imperfect physics pass along the currently recommended direction; the puck does not home toward the receiver, and obstruction, bounded error, player motion, interception, or a failed reception can prevent success.
- [x] One SHOOT action charges while held and releases on button-up; short holds release quickly at lower power, longer holds produce stronger bounded shots, player facing and goal assistance determine approximate direction without reading movement input directly, and configured inaccuracy permits misses.
...
- [x] Keyboard/gamepad Editor controls exercise movement independently from tap PASS, held shoot/release, and switch through the same action contract as touch without requiring any PASS drag gesture.
- [x] Automated smoke verification confirms the generated 5v5-plus-goalies roster, one-local-input invariant, modular movement/puck/AI/camera/match/UI wiring, score/reset behavior, and absence of networking/service packages or namespaces.
...
- Use five skaters and one goalie per team for Phase 1; keep roster and formation data count-driven.
...
- Line changes, penalties, offsides, icing, fighting, injuries, overtime, user-controlled goalies, advanced faceoffs, or NHL-level goalie simulation.
