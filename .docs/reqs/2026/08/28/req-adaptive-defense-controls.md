...
When a Red opponent possesses the puck, replace the Blue user's offensive touch actions with two defensive actions: SWITCH and CHECK. SWITCH must transfer control through the existing useful-defender selection path. CHECK must perform a contextual defensive challenge from the controlled Blue skater: a close-range body check or a longer stick-range pull check. Returning to human possession or a loose puck must restore the normal offensive controls.
...
- [x] Human possession and loose-puck play show the normal PASS, DEKE, and SHOOT actions, and repeated possession changes do not leave stale labels, hidden buttons, or held input behind.
...
- [x] Automated gameplay verification covers offensive/defensive control transitions, touch SWITCH routing, successful body and pull checks, rejected out-of-range checks, and restoration of offensive controls.
...
- AI-triggered checks, goalie checks, automatic checking, or new automatic player-switch policy.
